...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	 ...
	 */
	protected $collTransfers;

	/**
	 * @var        Criteria The criteria used to select the current contents of collTransfers.
	 */
	private $lastTransferCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collTransfers = null;
			$this->lastTransferCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collTransfers !== null) {
				foreach ($this->collTransfers as $referrerFK) {
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getTransfers() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addTransfer($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collTransfers collection (array).
	 *
	...
	{
		$this->collTransfers = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collTransfers collection (array).
	 *
	 * By default this just sets the collTransfers collection to an empty array (like clearcollTransfers());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initTransfers()
	{
		$this->collTransfers = array();
	}
	 ...
	 * related Transfers from storage. If this sfGuardUser is new, it will return
	 ...
	 * @throws     PropelException
	 */
	public function getTransfers($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collTransfers === null) {
			if ($this->isNew()) {
			   $this->collTransfers = array();
				...
				$criteria->add(TransferPeer::USER_ID, $this->id);

				TransferPeer::addSelectColumns($criteria);
				$this->collTransfers = TransferPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				TransferPeer::addSelectColumns($criteria);
				if (!isset($this->lastTransferCriteria) || !$this->lastTransferCriteria->equals($criteria)) {
					$this->collTransfers = TransferPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastTransferCriteria = $criteria;
		return $this->collTransfers;
	}
	 ...
	 * @throws     PropelException
	 */
	public function countTransfers(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collTransfers === null) {
			...
			if (!$this->isNew()) {
				...
				if (!isset($this->lastTransferCriteria) || !$this->lastTransferCriteria->equals($criteria)) {
					...
					$count = count($this->collTransfers);
				}
			} else {
				$count = count($this->collTransfers);
			}
	 ...
	 * @throws     PropelException
	 */
	public function addTransfer(Transfer $l)
	{
		if ($this->collTransfers === null) {
			$this->initTransfers();
		}
		if (!in_array($l, $this->collTransfers, true)) { // only add it if the **same** object is not already associated
			array_push($this->collTransfers, $l);
			$l->setUser($this);
		}
	}
	 ...
	 * been saved, it will retrieve related Transfers from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in sfGuardUser.
	 */
	public function getTransfersJoinAccountOrigin($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collTransfers === null) {
			if ($this->isNew()) {
				...
				$this->collTransfers = TransferPeer::doSelectJoinAccountOrigin($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * actually need in sfGuardUser.
	 */
	public function getTransfersJoinAccountDestination($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collTransfers === null) {
			if ($this->isNew()) {
				...
				$this->collTransfers = TransferPeer::doSelectJoinAccountDestination($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related Transactions from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collTransactions === null) {
			if ($this->isNew()) {
			   $this->collTransactions = array();
				...
				$criteria->add(TransactionPeer::USER_ID, $this->id);
	...
	{
		if ($deep) {
			if ($this->collTransfers) {
				foreach ((array) $this->collTransfers as $o) {
