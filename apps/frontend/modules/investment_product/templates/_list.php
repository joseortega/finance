<div class="list">
  ...
  <?php else: ?>
      ...
      <tr>
        <th colspan="4">
          <?php if ($pager->haveToPaginate()): ?>
            <?php include_partial('investment_product/pagination', array('pager' => $pager)) ?>
          <?php endif; ?>
    ...
    <tbody>
      ...
      <tr class="row">
        <td class="title"><a href="<?php echo url_for('investment_product_show', $product)?>"><?php echo $product->getName() ?></a></td>   
        <td class="count_accounts">
          <?php echo __('Investments')?>
          <?php echo $product->countInvestments()?>
        </td>
      </tr>
      <?php endforeach; ?>
    </tbody>
