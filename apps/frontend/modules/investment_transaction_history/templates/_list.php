...
<?php else: ?>
  <?php foreach ($pager->getResults() as $key=>$transaction): ?>
    <div class="alert <?php echo $transaction->getTypeOperationText()?>">
      <div class="body">
        <div class="title">
          <a href="<?php echo url_for('investment_transaction_history/show?id='.$investment->getId().'&transaction_id='.$transaction->getId())?>"><?php echo $transaction->getTransactionType()?></a>
          <?php echo __('type')?> 
  ...
  <?php if ($pager->haveToPaginate()): ?>
    <?php include_partial('pagination', array('pager' => $pager, 'investment'=>$investment)) ?>
  <?php endif; ?>
