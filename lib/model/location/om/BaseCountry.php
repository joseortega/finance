...
abstract class BaseCountry extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array Province[] Collection to store aggregation of Province objects.
	 */
	protected $collProvinces;

	/**
	 * @var        Criteria The criteria used to select the current contents of collProvinces.
	 */
	private $lastProvinceCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->lastProvinceCriteria = null;
	...
	/**
	 * Clears out the collProvinces collection (array).
	 *
	...
	/**
	 * Initializes the collProvinces collection (array).
	 *
	 ...
	 * related Provinces from storage. If this Country is new, it will return
		...
		if ($this->collProvinces === null) {
			if ($this->isNew()) {
			   $this->collProvinces = array();
				...
				$criteria->add(ProvincePeer::COUNTRY_ID, $this->id);

				ProvincePeer::addSelectColumns($criteria);
				$this->collProvinces = ProvincePeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				ProvincePeer::addSelectColumns($criteria);
				if (!isset($this->lastProvinceCriteria) || !$this->lastProvinceCriteria->equals($criteria)) {
					$this->collProvinces = ProvincePeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastProvinceCriteria = $criteria;
	...
	public function countProvinces(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collProvinces === null) {
			if ($this->isNew()) {
				...
				$count = ProvincePeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastProvinceCriteria) || !$this->lastProvinceCriteria->equals($criteria)) {
					...
					$count = count($this->collProvinces);
				}
			} else {
				$count = count($this->collProvinces);
			}
	...
	public function addProvince(Province $l)
	{
		...
		if (!in_array($l, $this->collProvinces, true)) { // only add it if the **same** object is not already associated
			array_push($this->collProvinces, $l);
			$l->setCountry($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collProvinces) {
				foreach ((array) $this->collProvinces as $o) {
